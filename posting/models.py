...
class Feed(models.Model):
    ...
    user_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='feed_user_id') # FK, related_name을 id값으로 식별
...
class Feed_image(models.Model):
    feed = models.ForeignKey(Feed, on_delete=models.CASCADE,related_name='image_feed_id') # 다대일 관계 지정
...
class Comment(models.Model):
    feed_id = models.ForeignKey(Feed, on_delete=models.CASCADE, related_name='comment_feed_id') # 다대일 관계 지정
    user_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='comment_user_id') # FK, related_name을 id값으로 식별
...
class Comment_image(models.Model):
    comment = models.ForeignKey(Comment, on_delete=models.CASCADE,related_name='image_comment_id') # 다대일 관계 지정
    image = models.ImageField(default='media/posting/default_image.jpeg', upload_to='posting', blank=True, null=True)
